...
const StrategicResolutionEngine: React.FC = () => (
  <div className="relative w-full h-[400px] lg:h-[600px] flex items-center justify-center animate-materialize select-none">
    {/* Dynamic Background Mesh - Spans the full 60% container */}
    <div className="absolute inset-0 overflow-hidden pointer-events-none opacity-30">
      <svg width="100%" height="100%" viewBox="0 0 800 600" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMidYMid slice">
        ...
        <path d="M100 100 L250 120 L200 300 L50 250 Z" stroke="#4F46E5" strokeWidth="0.5" strokeOpacity="0.1" fill="none">
           <animate attributeName="d" values="M100 100 L250 120 L200 300 L50 250 Z; M120 80 L280 140 L220 320 L70 270 Z; M100 100 L250 120 L200 300 L50 250 Z" dur="25s" repeatCount="indefinite" />
        </path>
      </svg>
    </div>
    ...
    <svg width="600" height="500" viewBox="0 0 600 500" fill="none" xmlns="http://www.w3.org/2000/svg" className="relative z-10 w-full h-full overflow-visible">
      {/* Infrastructure Gears */}
      <circle cx="300" cy="250" r="220" stroke="#1E293B" strokeWidth="0.5" strokeDasharray="4 20" className="animate-[spin_50s_linear_infinite]" />
      <circle cx="300" cy="250" r="190" stroke="#4F46E5" strokeWidth="1" strokeOpacity="0.1" strokeDasharray="2 30" className="animate-[spin_35s_linear_infinite_reverse]" />
      
      {/* Central Resolution Lattice - High Density Core */}
      <g className="animate-float">
        <path d="M300 120 L400 180 V320 L300 380 L200 320 V180 L300 120Z" fill="white" stroke="#1E293B" strokeWidth="1" />
        <path d="M300 160 L360 200 V300 L300 340 L240 300 V200 L300 160Z" stroke="#4F46E5" strokeWidth="2" strokeOpacity="0.4" fill="#4F46E5" fillOpacity="0.05" />
        
        {/* Core Pulsing Unit */}
        <circle cx="300" cy="250" r="15" fill="#4F46E5" className="animate-pulse" />
        <circle cx="300" cy="250" r="50" stroke="#4F46E5" strokeWidth="0.5" strokeOpacity="0.2">
      ...
      <g>
        {[...Array(6)].map((_, i) => (
          <g key={i} transform={`rotate(${i * 60} 300 250)`}>
             <circle cx="300" cy="60" r="4" fill="#1E293B" />
             <line x1="300" y1="60" x2="300" y2="120" stroke="#1E293B" strokeWidth="0.5" strokeDasharray="2 2" />
          </g>
        ))}
      </g>
      ...
      <g>
        {[...Array(15)].map((_, i) => (
          <circle key={i} r="1.5" fill="#4F46E5">
            <animateMotion 
              path={`M-50 ${100 + i * 25} Q 150 ${250} 300 250`} 
              dur={`${3 + i * 0.4}s`} 
      ...
      {/* Structured Output (Radiating) */}
      <g>
        {[...Array(8)].map((_, i) => (
          <rect key={i} width="5" height="5" fill="#4F46E5" transform="rotate(45)">
            {/* Fix: Moved 's' unit outside the expression braces to avoid potential syntax error */}
            <animateMotion 
              path={`M300 250 Q 450 ${250} 650 ${100 + i * 35}`} 
              dur={`${4 + i * 0.3}s`} 
              repeatCount="indefinite" 
              begin={`${i * 0.2}s`}
            />
      ...
      {/* Center Brand Mark Mark */}
      <g transform="translate(285, 235) scale(0.25)" opacity="0.6">
         <path d="M10 5 V40 H30 Q25 40 20 35 Q15 30 15 25 H10" fill="white" />
         <path d="M25 5 L55 50 H65 V5 H55 V40 L25 5Z" fill="white" />
      </g>
    </svg>

    {/* System Telemetry Labels - Anchored to corners of visual space */}
...
const DataConstellation: React.FC = () => (
  <div className="absolute inset-0 pointer-events-none overflow-hidden opacity-30">
    <svg className="absolute inset-0 w-full h-full">
      <defs>
        <pattern id="deep-grid" width="120" height="120" patternUnits="userSpaceOnUse">
          ...
          <circle cx="0" cy="0" r="1" fill="rgba(79, 70, 229, 0.1)" />
        </pattern>
      </defs>
    ...
    {[...Array(24)].map((_, i) => (
      <div 
        key={i}
        className="absolute w-px h-64 bg-gradient-to-b from-transparent via-indigo-500/5 to-transparent animate-scan-v"
        style={{
          left: `${(i + 1) * 4}%`,
...
const Home: React.FC<HomeProps> = ({ onNavigate, avatar }) => {
  return (
    <div className="flex flex-col overflow-hidden bg-white">
      {/* 🚀 ELITE HERO SECTION - REBALANCED 40/60 SPLIT */}
      <section className="relative pt-48 pb-40 px-4 overflow-hidden min-h-[95vh] flex flex-col items-center">
        ...
        <div className="max-w-7xl mx-auto flex flex-col lg:flex-row items-center justify-between relative z-10 w-full gap-24">
          ...
          <div className="w-full lg:w-[40%] text-center lg:text-left space-y-12 animate-fade-in-up">
            ...
            <div className="flex flex-col sm:flex-row justify-center lg:justify-start space-y-5 sm:space-y-0 sm:space-x-10">
              ...
              <button 
                ...
                className="px-14 py-6 bg-white text-slate-900 border border-slate-200 font-black uppercase tracking-[0.3em] text-[11px] rounded-2xl hover:bg-slate-50 transition-all transform hover:-translate-y-2 active:scale-95"
              >
                Talk to Us
              </button>
            </div>
          ...
          {/* Right Side: Visual Data Command (60%) */}
        ...
        <div className="hidden lg:flex absolute bottom-12 left-0 right-0 justify-center space-x-24 opacity-30 select-none">
           {['Traceable', 'Deterministic', 'Scaleable', 'Human-led'].map(attr => (
             <span key={attr} className="text-[10px] font-black uppercase tracking-[0.5em] text-slate-400">{attr}</span>
           ))}
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-4">
          <div className="flex flex-col md:flex-row justify-between items-end mb-24 animate-fade-in">
            <div className="max-w-2xl">
              ...
              <h2 className="text-5xl font-bold tracking-tighter mb-6 leading-none">Systems for Scale</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {PRODUCTS.map((product, index) => (
              <div 
                ...
                className={`group relative bg-white/5 border border-white/10 p-12 rounded-[4rem] hover:bg-white hover:text-slate-900 transition-all duration-500 cursor-pointer overflow-hidden animate-fade-in-up delay-${(index + 1) * 100}`}
              >
                <div className="relative z-10">
                  <div className="w-14 h-14 bg-indigo-600 rounded-[1.5rem] flex items-center justify-center font-black text-2xl mb-10 group-hover:scale-110 transition-transform">
                    {product.name[0]}
                  </div>
                  <h3 className="font-bold text-3xl mb-4 tracking-tighter">{product.name}</h3>
                  <p className="text-slate-400 text-sm leading-relaxed mb-10 group-hover:text-slate-600 transition-colors font-light">
                    {product.tagline}
                  </p>
                  <div className="text-[10px] font-black uppercase tracking-widest opacity-0 group-hover:opacity-100 transition-all translate-y-4 group-hover:translate-y-0 duration-300">
                    Explore Protocol &rarr;
                  </div>
                </div>
                <span className="absolute -right-6 -bottom-10 text-[11rem] font-black text-white/5 pointer-events-none group-hover:text-slate-100 transition-colors select-none">
                  0{index + 1}
                </span>
              </div>
            ))}
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-24 items-center">
          <div className="animate-fade-in-up">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
              <div className="p-10 bg-slate-50 rounded-[3rem] border border-slate-100 group hover:border-indigo-200 transition-all">
                <h4 className="font-bold text-slate-900 mb-3 tracking-tight">Deterministic QA</h4>
                <p className="text-sm text-slate-500 font-light leading-relaxed">Programmatic validation at every stage of ingestion.</p>
              </div>
              <div className="p-10 bg-slate-50 rounded-[3rem] border border-slate-100 group hover:border-indigo-200 transition-all">
                <h4 className="font-bold text-slate-900 mb-3 tracking-tight">Audit Trails</h4>
                <p className="text-sm text-slate-500 font-light leading-relaxed">Every decision traceable to its source extraction.</p>
              </div>
            </div>
          </div>
          ...
          <div className="relative flex justify-center lg:justify-end animate-float">
             ...
             <div className="bg-slate-900 rounded-[5rem] p-16 text-white relative shadow-[0_50px_100px_-20px_rgba(0,0,0,0.5)] border border-white/5 overflow-hidden group w-full max-w-xl">
                <div className="flex items-center justify-between mb-16">
                   <div className="bg-indigo-600 inline-block px-5 py-2 rounded-xl text-[10px] font-black tracking-widest uppercase">CORE STATUS: ACTIVE</div>
                ...
                <div className="flex items-center justify-center space-x-10 relative z-10">
                  ...
                  <div className="text-center">
                    <div className="font-black text-2xl tracking-tighter">NESTOR</div>
                ...
                <div className="absolute -right-24 -bottom-24 opacity-[0.03] select-none pointer-events-none transition-transform duration-700">
                   <span className="text-[22rem] font-black tracking-tighter">NEST</span>
                </div>
             </div>
          </div>
        </div>
