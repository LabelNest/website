        ...
        {Object.entries(TEAM_BY_DEPT).map(([dept, members]) => (
          <div key={dept} className="mb-24">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              {members.map((member) => (
                <div key={member.id} className="bg-white rounded-[3.5rem] border border-slate-200 p-10 flex flex-col md:flex-row gap-10 hover:shadow-2xl hover:shadow-indigo-100 transition-all group overflow-hidden relative">
                  <div className="w-44 h-44 bg-slate-900 rounded-[2.5rem] flex-shrink-0 flex items-center justify-center overflow-hidden border border-white/10 relative">
                    <img 
                      src={getTeamMemberAvatar(member.id, dept)} 
                      alt={member.name} 
                  ...
                  <div className="flex-1">
                    ...
                    <ul className="space-y-3 mb-8 text-slate-800 text-sm leading-relaxed font-light">
                      {member.bio.split(/(?<=[.!?])\s+/).filter(s => s.trim().length > 0).map((sentence, idx) => (
          ...
          {isAlumniOpen && (
            <div className="mt-16 bg-slate-50 rounded-[4rem] border border-slate-100 overflow-hidden animate-in fade-in slide-in-from-top-4 duration-500">
              <div className="p-12 space-y-4">
                <div className="grid grid-cols-12 gap-8 text-[10px] font-black uppercase tracking-widest text-slate-400 px-4 mb-8">
                  <div className="col-span-4">Legacy Asset</div>
                  <div className="col-span-4">Tenure Role</div>
                  <div className="col-span-4">Impact Vector</div>
                </div>
                
                <div className="space-y-2">
                  {ALUMNI.map((alumnus, i) => (
                    <div key={i} className="grid grid-cols-12 gap-8 p-6 bg-white border border-slate-100 rounded-2xl items-center hover:border-indigo-200 transition-colors">
                      <div className="col-span-4 text-slate-900 font-bold tracking-tight">{alumnus.name}</div>
                      <div className="col-span-4 text-slate-500 text-xs font-light">{alumnus.role}</div>
                      <div className="col-span-4 text-indigo-600 text-[10px] font-black uppercase tracking-widest leading-relaxed">
                        {alumnus.impact}
                      </div>
                    </div>
                  ))}
